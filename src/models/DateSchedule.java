...
//        System.out.println("number of num:" + count + "/" + list.size());
    ...
    public List<ClassRoom> getClassRoomTHList() throws IOException {
        ...
        while (line != null) {
            String[] tokens = line.split(",");
            classRooms.add(new ClassRoom(tokens[0], tokens[1], Integer.parseInt(tokens[2]), Integer.parseInt(tokens[3]), Integer.parseInt(tokens[4]), Integer.parseInt(tokens[5])));//add building id
            line = reader.readLine();
        }
        return classRooms;
    }
    ...
    public List<RegistrationClass> getRegistrationClass() throws IOException {
        ...
        while (line != null) {
            ...
            for (Subject s : subjectList) {
                if (s.getId().equals(tokens[1].substring(0, tokens[1].lastIndexOf("-")))) {
                    subject = new Subject(s.getId(), s.getName(), s.getCredit(), s.getExamForms(), s.getExamTime(), s.getLessonTime());
                    break;
                }
            }
            if (subject != null)
                classRooms.add(new RegistrationClass(tokens[0], tokens[2], Integer.parseInt(tokens[3]), Integer.parseInt(tokens[4]), subject, new Grade(tokens[6], tokens[5])));
            line = reader.readLine();
        }
        return classRooms;
    }
